...
{
    QSettings settings = getSettings();
    return settings.value("DL/pythonEnvironment", "").toString();
}

void AppSettings::setPythonEnvironment(const QString &path)
{
    QSettings settings = getSettings();
    settings.setValue("DL/pythonEnvironment", path);
    settings.sync();
}
...
{
    QSettings settings = getSettings();
    return settings.value("DL/defaultConfThreshold", 0.25f).toFloat();
}

void AppSettings::setDefaultConfThreshold(float value)
{
    QSettings settings = getSettings();
    settings.setValue("DL/defaultConfThreshold", value);
    settings.sync();
}
...
{
    QSettings settings = getSettings();
    return settings.value("DL/defaultIOUThreshold", 0.45f).toFloat();
}

void AppSettings::setDefaultIOUThreshold(float value)
{
    QSettings settings = getSettings();
    settings.setValue("DL/defaultIOUThreshold", value);
    settings.sync();
}
...
{
    QSettings settings = getSettings();
    return settings.value("DL/defaultImageSize", 640).toInt();
}

void AppSettings::setDefaultImageSize(int size)
{
    QSettings settings = getSettings();
    settings.setValue("DL/defaultImageSize", size);
    settings.sync();
}
