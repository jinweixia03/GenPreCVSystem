    ...
    , m_batchProcessDialog(nullptr)
{
    ...
    logMessage("提示：切换到\"任务\"→\"目标检测\"或\"语义分割\"可使用 DL 推理功能");
}
...
void MainWindow::on_actionBatchProcess_triggered()
{
    if (!m_taskController || !m_taskController->dlService()) {
        QMessageBox::warning(this, tr("提示"), tr("请先启动 DL 服务"));
        return;
    }
    ...
    // 设置 DL 服务（对话框会自动处理模型和任务选择）
    m_batchProcessDialog->setDLService(m_taskController->dlService());
...
{
    // 委托给 TaskController 处理（包含 DL 服务控件和信号连接）
    if (m_taskController) {
