...
bool ExportService::exportDetectionResult(const DetectionResult &result,
...
bool ExportService::exportClassificationResult(const ClassificationResultList &result,
                                                const ExportMetadata &metadata,
                                                const QString &filePath,
                                                Format format)
{
...
bool ExportService::exportKeypointResult(const KeypointResult &result,
                                          const ExportMetadata &metadata,
                                          const QString &filePath,
                                          Format format)
{
    if (format == Format::JSON) {
        ...
        QJsonArray detectionsArray;
        for (const KeypointDetection &det : result.detections) {
            QJsonObject detObj;
...
bool ExportService::exportBatchResults(const QVector<QPair<QString, DetectionResult>> &results,
                                        ...
                                        Format format)
{
    if (format == Format::JSON) {
        ...
        for (const auto &pair : results) {
            ...
            const DetectionResult &result = pair.second;
        ...
        for (const auto &pair : results) {
            ...
            for (const Detection &det : pair.second.detections) {
...
QJsonObject ExportService::detectionToJson(const Detection &det)
{
    QJsonObject obj;
...
QString ExportService::detectionToCsvRow(const Detection &det, const ExportMetadata &metadata)
{
