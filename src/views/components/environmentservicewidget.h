...
public:
     ...
     * @brief 设置 DL 服务
     */
    void setDLService(Utils::DLService *service);
...
private slots:
    ...
    void onReloadEnvironments();  // 重新检测环境
    void onScanProgress(const QString &message);  // 扫描进度
    void onScanCompleted(const QVector<Utils::CachedEnvironment> &environments);  // 扫描完成
    void onScanFailed(const QString &errorMessage);  // 扫描失败
...
private:
     ...
     * @param task 任务类型
     * @return 模型文件路径，未找到返回空字符串
     */
    QString findFirstAvailableModel(Models::CVTask task) const;
    ...
    // 环境扫描器（同步扫描，在后台线程中执行）
    Utils::EnvironmentScanner *m_envScanner;
    // 扫描结果缓存
    QVector<Utils::CachedEnvironment> m_scannedEnvironments;
};
